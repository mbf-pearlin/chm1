...
function trim (el) {
  ...
  $(function(){
   ...
   $(".tabs .tabs-links a").on("click",function(t){
    ...
    jQuery( document ).on('click',"#contact_us_formbk",function() {
       var name=jQuery('#bk_firstname').val();
       var email=jQuery('#bk_email').val();
       var dob=jquery('#bk_dob').val();
       var age=jquery('#bk_age').val();
       var telephone=jQuery('#bk_telephone').val();
       var gender=jQuery('#bk_gender').val().trim();
       var message=jQuery('#bk_message').val().trim();
       var existingCustomer=jQuery('input[name=existing_customer]:checked', '#contact_us_frmbk').val();
       var regex = /^([a-z\d!#$%&'*+\-\/=?^_`{|}~\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF]+(\.[a-z\d!#$%&'*+\-\/=?^_`{|}~\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF]+)*|"((([ \t]*\r\n)?[ \t]+)?([\x01-\x08\x0b\x0c\x0e-\x1f\x7f\x21\x23-\x5b\x5d-\x7e\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF]|\\[\x01-\x09\x0b\x0c\x0d-\x7f\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF]))*(([ \t]*\r\n)?[ \t]+)?")@(([a-z\d\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF]|[a-z\d\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF][a-z\d\-._~\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF]*[a-z\d\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])\.)+([a-z\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF]|[a-z\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF][a-z\d\-._~\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF]*[a-z\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])\.?$/i;
       var x=0;
       console.log(x);
        if (name=='' || name == undefined) {
           jQuery('#err_bk_firstname').parents('.form-row').addClass('error-row');
           jQuery('#err_bk_firstname').text("Please enter your name").show();

           x++;

        } else {
           jQuery('#err_bk_firstname').parents('.form-row').removeClass('error-row');
           jQuery('#err_bk_firstname').hide();
        }
        if (email!='') {
           if (!regex.test(email)) {
               jQuery('#err_login_bk_email').hide();
               jQuery('#err_login_bk_email').parents('.form-row').addClass('error-row');
               jQuery('#err_login_bk_email').text("Please enter a valid email address").show();
               x++;
           } else {
               jQuery('#err_login_bk_email').parents('.form-row').removeClass('error-row');
               jQuery('#err_login_bk_email').hide();
           }
        } else {
           jQuery('#err_login_bk_email').hide();
           jQuery('#err_login_bk_email').text("Please enter your email address").show();
           jQuery('#err_login_bk_email').parents('.form-row').addClass('error-row');
           x++;
        }
        if (dob=='' || dob == undefined) {
           jQuery('#err_bk_dob').parents('.form-row').addClass('error-row');
           jQuery('#err_bk_dob').text("Please enter your dob").show();
           x++;
        } else {
           jQuery('#err_bk_dob').parents('.form-row').removeClass('error-row');
           jQuery('#err_bk_dob').hide();
        }
        if (age=='' || age == undefined) {
           jQuery('#err_bk_age').parents('.form-row').addClass('error-row');
           jQuery('#err_bk_age').text("Please enter your age").show();
           x++;
        } else {
           jQuery('#err_bk_age').parents('.form-row').removeClass('error-row');
           jQuery('#err_bk_age').hide();
        }
     if (gender=='' || gender == undefined) {
           jQuery('#err_bk_gender').parents('.form-row').addClass('error-row');
           jQuery('#err_bk_gender').text("Please enter gender").show();
           x++;
        } else {
           jQuery('#err_bk_gender').parents('.form-row').removeClass('error-row');
           jQuery('#err_bk_gender').hide();
        }
        if (telephone=='' || telephone == undefined) {
           jQuery('#err_bk_telephone').parents('.form-row').addClass('error-row');
           jQuery('#err_bk_telephone').text("Please enter your phone number").show();
           x++;
        } else {
           jQuery('#err_bk_telephone').parents('.form-row').removeClass('error-row');
           jQuery('#err_bk_telephone').hide();
        }
        if (message=='' || message ==undefined) {
           jQuery('#err_bk_message').parents('.form-row').addClass('error-row');
           jQuery('#err_bk_message').text("Please enter your message").show();
           x++;
        } else {
             jQuery('#err_bk_message').parents('.form-row').removeClass('error-row');
           jQuery('#err_bk_message').hide();
        }
        ...
        jQuery('#err_bk_terms').parents('.checkboxradio-row').removeClass('error-row');
        jQuery('#err_bk_terms').hide();

        } else {

        console.log('here1');
        jQuery('#err_bk_terms').parents('.checkboxradio-row').addClass('error-row');
        jQuery('#err_bk_terms').text("Please agree the terms and condition").show();
    ...
    $('.checkboxradio-row').fadeIn();
  });
    $('.checkboxradio-row').hide();
/* booking form */
    ...
    jQuery( document ).on('click',"#contact_book_submit",function() {
       ...
       console.log(x);
        if (fname=='' || fname == undefined) {
           ...
           jQuery('#bookerr_firstname').text("Please enter your Child's name1").show();
           x++;
        } else {
